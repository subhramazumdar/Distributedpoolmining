	   ...
	   // igraph_vector_int_print(&mdeg);
	    ...
	    for(j=0;j<igraph_vcount(g);j++)
	    {
	     for(i=0;i<igraph_vcount(g);i++)
	     	{
	     	//printf("%d,%d\n",j,visited[i]);
	     	if(visited[i]==0 && VECTOR(v)[j]==VECTOR(mdeg)[i])
	     	{
	     		
	     		
	     		vertex_id[j]=i;
	     		//printf("%d,%d,%d\n",i,j,igraph_vector_get(&indices,j));
	    ...
	    int count2=0;	
	     ...
	     igraph_vs_t vs1;
	     igraph_vs_adj(&vs2, vertex_id[igraph_vcount(g)-1], IGRAPH_OUT);
	     igraph_vit_create(g, vs2, &vit2);
	     ...
	     igraph_vit_t vit3;
	     igraph_vs_t vs3;
	     #pragma omp parallel for num_threads(10) shared(visited,vertex_id) private(vs3,vit3,j) 
	     for(j=0;j<igraph_vcount(g);j++)
	     {
		 ...
		 // #pragma omp critical (visited)
		  visited[vertex_id[j]]=1;
		  //printf("v:%d\n",vertex_id[j]);
		}
		  printf("%d,%d\n",vertex_id[j],j);
	         igraph_vs_adj(&vs3, vertex_id[j], IGRAPH_OUT);
